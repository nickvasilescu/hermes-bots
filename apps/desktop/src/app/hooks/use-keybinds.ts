...
export function useKeybinds(deps: KeybindRuntimeDeps): void {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // Contextual surfaces can claim a built-in chord before it mutates app
      // state. Runtime plugins load after this capture listener, so a second
      // keydown listener can only undo the action (and visibly flickers).
      // A cancelable pre-dispatch event gives them a real interception seam.
      if (!keybindActionAllowed(actionId, combo)) {
        event.preventDefault()
        event.stopPropagation()

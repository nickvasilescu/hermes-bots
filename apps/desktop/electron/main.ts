...
import {
  ...
  COMPOSIO_MCP_TIMEOUT_SECONDS,
  COMPOSIO_MCP_TRUST,
  type ComposioBroker,
  type ComposioMcpEntry,
  createBrokerFromStoreIo,
  createSdkClient,
  publicError
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!IS_MAC) {
    ...
    windowsSandboxIntegration?.appendUpdaterFallback(args, relaunchArgs)
...
function composioStorePath() {
  return path.join(app.getPath('userData'), 'composio-connectors.json')
}

function _composioStoreIo() {
  return {
    encrypt: encryptDesktopSecret,
    decrypt: decryptDesktopSecret,
    readStoreText: () => fs.readFileSync(composioStorePath(), 'utf8'),
    writeStoreText: text => {
      fs.mkdirSync(path.dirname(composioStorePath()), { recursive: true })
      fs.writeFileSync(composioStorePath(), text, { mode: 0o600 })
    },
    rememberLog
  }
}

let _composioBroker: ComposioBroker | null = null
...
async function upsertComposioMcp(profile, entry: ComposioMcpEntry) {
...
async function removeComposioMcp(profile) {
  try {
    await composioProfileApi(profile, {
      path: `/api/mcp/servers/${encodeURIComponent(COMPOSIO_MCP_SERVER_NAME)}`,
      method: 'DELETE'
    })
  } catch (error) {
...
function getComposioBroker() {
  if (!_composioBroker) {
    _composioBroker = createBrokerFromStoreIo(_composioStoreIo(), {
      createClient: createSdkClient,
      openExternal: async url => {
        if (!openExternalUrl(url)) {
          throw new Error('Unable to open the connector consent page.')
        }
      },
      mcp: {
        upsert: upsertComposioMcp,
        remove: removeComposioMcp
      },
      randomUuid: () => crypto.randomUUID()
    })
  }

  return _composioBroker
}

async function withComposioBroker(run) {
  try {
    return await run(getComposioBroker())
  } catch (error) {
...
// The default Orgo desktop binding is app-wide and inherited by every agent.
// Profiles may still save an explicit override. The API key is always encrypted
// with the OS keychain-backed safeStorage primitive; the renderer only receives
// whether a key is present, never the key itself. VNC credentials are
// intentionally not persisted because Orgo rotates them when a computer restarts.
interface SavedOrgoDesktopConfig {
  version: 1
  profiles: Record<
    string,
    {
      apiKey: { encoding: string; value: string }
      computerId: string
      workspaceId?: string
    }
  >
...
function orgoDesktopProfileKey(profile) {
  const key = String(profile ?? '').trim() || 'default'

  if (key !== 'default' && !PROFILE_NAME_RE.test(key)) {
    throw new Error('Invalid Hermes profile for the Orgo desktop.')
  }

  return key
}

function readOrgoDesktopConfig(): SavedOrgoDesktopConfig {
  const empty: SavedOrgoDesktopConfig = { version: 1, profiles: {} }

  try {
    tightenSecretFileMode(DESKTOP_ORGO_CONFIG_PATH)
    const parsed = JSON.parse(fs.readFileSync(DESKTOP_ORGO_CONFIG_PATH, 'utf8'))

    if (!parsed || typeof parsed !== 'object' || !parsed.profiles || typeof parsed.profiles !== 'object') {
      return empty
    }

    const profiles: SavedOrgoDesktopConfig['profiles'] = {}

    for (const [rawProfile, rawEntry] of Object.entries(parsed.profiles)) {
      let profile
      let computerId
      ...
      const apiKey = (rawEntry as any)?.apiKey

      if (!apiKey || apiKey.encoding !== SAFE_STORAGE_ENCODING || typeof apiKey.value !== 'string' || !apiKey.value) {
        continue
      }
      ...
      profiles[profile] = {
        apiKey,
        computerId,
        workspaceId: String((rawEntry as any)?.workspaceId || '').trim()
      }
    }
...
function writeOrgoDesktopConfig(config) {
  fs.mkdirSync(path.dirname(DESKTOP_ORGO_CONFIG_PATH), { recursive: true })
...
async function listOrgoSyncProfiles() {
  try {
    const payload = (await composioProfileApi('default', { path: '/api/profiles', method: 'GET' })) as {
...
function sanitizeOrgoDesktopConfig(profile) {
  ...
  const { entry, inheritedFromDefault } = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, key)

  return {
    configured: Boolean(entry?.computerId && entry?.apiKey?.value),
...
function saveOrgoApiKey(apiKeyValue) {
  ...
  const config = readOrgoDesktopConfig()
  const current = resolveOrgoDesktopProfile(config.profiles, 'default').entry
  config.profiles.default = {
    apiKey: encryptDesktopSecret(apiKey),
    computerId: current?.computerId || '',
    workspaceId: current?.workspaceId || ''
  }
  writeOrgoDesktopConfig(config)

  return sanitizeOrgoDesktopConfig('default')
}

async function provisionOrgoSharedComputer() {
  const current = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, 'default').entry
  const apiKey = decryptDesktopSecret(current?.apiKey)

  if (!apiKey) {
    throw new Error('Enter an Orgo API key.')
  }

  let workspaceId = String(current?.workspaceId || '').trim()
...
function localTailscaleCliPath(): string {
  const candidates =
...
function localTailscaleStatus() {
  const cliPath = localTailscaleCliPath()
  ...
  try {
    const output = execFileSync(cliPath, ['status', '--json'], {
      encoding: 'utf8',
      timeout: 8_000,
      windowsHide: true
    })
...
async function openLocalTailscale() {
  ...
  if (process.platform === 'darwin' && fs.existsSync(appPath)) {
    ...
    return { opened: !error, installed: true, error }
  }

  await openExternalUrl('https://tailscale.com/download')

  return { opened: true, installed: false, error: '' }
}

function defaultOrgoDesktopCredentials() {
  const entry = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, 'default').entry
  const apiKey = decryptDesktopSecret(entry?.apiKey)
  const computerId = String(entry?.computerId || '').trim()

  if (!apiKey || !computerId) {
    throw new Error('Create the shared Orgo computer first.')
  }

  return { apiKey, computerId }
}

async function beginOrgoTailscale() {
  const { apiKey, computerId } = defaultOrgoDesktopCredentials()
  const status = await beginOrgoTailscaleSetup(apiKey, computerId)

  if (status.authUrl) {
    await openExternalUrl(status.authUrl)
  }

  return status
}

async function orgoTailscaleStatus() {
  const { apiKey, computerId } = defaultOrgoDesktopCredentials()

  return getOrgoTailscaleStatus(apiKey, computerId)
}
...
async function createOrgoDesktopSession(profile) {
  ...
  if (!entry?.computerId || !apiKey) {
    return {
      ok: false,
      error: { code: 'invalid-config', message: 'Connect an Orgo computer first.' }
    }
  }

  try {
    await ensureOrgoComputerRunning(apiKey, entry.computerId)

    return { ok: true, ...(await fetchOrgoDesktopSession({ apiKey, computerId: entry.computerId })) }
  } catch (error) {
    return { ok: false, error: serializeOrgoDesktopError(error) }
  }
}
...
function createWindow() {
  ...
  const revealController = wireWindowReveal(createdMainWindow, {
    onRevealed: () => {
      ...
      windowsSandboxIntegration?.markWindowRevealed(rememberLog)
    }
  })
  ...
  installWindowRendererLifecycle(mainWindow, {
    ...
    callbacks: {
      ...
      onCrashLoopSuppressed: details => {
        windowsSandboxIntegration?.handleRendererCrashLoop(details, rememberLog)
      }
    },
    reloadWindowMs: RENDERER_RELOAD_WINDOW_MS,
    reloadMax: RENDERER_RELOAD_MAX,
...
if (process.env.HERMES_DESKTOP_SKU !== 'bot-ssh-only') {
  registerSkuIntegrations?.(() => {
    ...
    ipcMain.handle('hermes:orgo-desktop:provision', async () => provisionOrgoSharedComputer())
    ipcMain.handle('hermes:orgo-desktop:tailscale:local-status', async () => localTailscaleStatus())
    ipcMain.handle('hermes:orgo-desktop:tailscale:local-open', async () => openLocalTailscale())
    ipcMain.handle('hermes:orgo-desktop:tailscale:begin', async () => beginOrgoTailscale())
    ipcMain.handle('hermes:orgo-desktop:tailscale:status', async () => orgoTailscaleStatus())
    ipcMain.handle('hermes:orgo-desktop:tailscale:connect', async () => connectHermesThroughOrgoTailscale())
    ipcMain.handle('hermes:orgo-desktop:ensure-running', async () => {
      const entry = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, 'default').entry
      const apiKey = decryptDesktopSecret(entry?.apiKey)

      if (!entry?.computerId || !apiKey) {
        throw new Error('Connect an Orgo computer first.')
      }
    ...
    ipcMain.handle('hermes:orgo-desktop:doctor', async () => {
      const entry = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, 'default').entry
      const apiKey = decryptDesktopSecret(entry?.apiKey)
    ...
    ipcMain.handle('hermes:connectors:key:save', async (_event, key) =>
      withComposioBroker(broker => broker.saveKey(key))
    )
...
ipcMain.handle('hermes:connection-config:apply', async (_event, payload) => applyDesktopConnectionConfig(payload))
...
app.on('before-quit', event => {
  ...
  windowsSandboxIntegration?.markCleanQuit()

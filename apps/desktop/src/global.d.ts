  ...
  interface Window {
    hermesDesktop: {
      ...
      orgoDesktop: {
        ...
        provision: () => Promise<DesktopOrgoConfig>
        ensureRunning: () => Promise<{ ok: boolean; computer?: { id: string; name: string; status: string } }>
        doctor: () => Promise<{
          ok: boolean
          apiAuth: boolean
      ...
      connectors: {
        keyStatus: () => Promise<DesktopComposioKeyStatus>
        saveKey: (key: string) => Promise<DesktopComposioKeyStatus>
        removeKey: () => Promise<DesktopComposioKeyStatus>
        catalog: (query?: DesktopComposioCatalogQuery) => Promise<DesktopComposioCatalog>
        categories: () => Promise<DesktopComposioCategory[]>
        connections: () => Promise<DesktopComposioConnection[]>
        authorize: (slug: string) => Promise<DesktopComposioAuthorizeResult>
        poll: (slug: string) => Promise<DesktopComposioAuthorizeResult>
        disconnect: (slug: string) => Promise<{ slug: string; status: DesktopComposioStatus }>
        syncProfiles: (profiles: string[]) => Promise<DesktopComposioSyncResult>
      }
...
export type DesktopComposioStatus = 'available' | 'pending' | 'connected' | 'error' | 'disconnected'

export interface DesktopComposioKeyStatus {
  configured: boolean
  hint: string | null
}

export interface DesktopComposioCatalogQuery {
  search?: string
  category?: string
  cursor?: string
}

export interface DesktopComposioToolkit {
  slug: string
  name: string
  description: string
  logo: string | null
...
export interface DesktopComposioCatalog {
  items: DesktopComposioToolkit[]
  nextCursor: string | null
}

export interface DesktopComposioCategory {
  id: string
  name: string
}

export interface DesktopComposioConnection {
  slug: string
  name: string
  description: string
  logo: string | null
  category: string
  status: DesktopComposioStatus
  accountId: string | null
  statusReason: string | null
}

export interface DesktopComposioAuthorizeResult {
  slug: string
  status: DesktopComposioStatus
  accountId: string | null
  opened: boolean
...
export interface DesktopComposioSyncResult {
  synced: number
  removed: number
  toolkits: string[]
}

export interface DesktopOrgoConfig {
  configured: boolean
  computerId: string
  workspaceId?: string
  apiKeySet: boolean
...
export type DesktopOrgoSessionResult =
  ...
  | {
      ok: false
      error: { code: DesktopOrgoErrorCode; message: string }
    }

...
const PRODUCT_TOOL_CARDS = new Set(['clarify', 'setup_mcp'])

/**
 * Product chat is intentionally final-answer-first. Ordinary tool calls stay
 * in the message repository for recovery and the explicit technical view, but
 * do not become transcript rows. Only output that is itself the result (an
 * image) or a question that blocks the run earns a persistent card in the calm
 * product transcript. Recoverable tool failures remain diagnostics; a failed
 * turn is surfaced once through MessagePrimitive.Error instead of once per
 * attempted tool.
 */
export function shouldPresentProductTool(part: ToolPart): boolean {
  if (PRODUCT_TOOL_CARDS.has(part.toolName)) {
    return true
  }
...
const PresentedTextPart: FC = () => {
  const toolViewMode = useStore($toolViewMode)

  // A running text part is still an answer draft, so product chat buffers it.
  // A sealed interim is different: Hermes emitted it deliberately through
  // message.interim as user-facing commentary ("On it, checking now."). Keep
  // that real conversational update while the separate activity reel handles
  // replace-in-place tool/thought telemetry underneath it.
  const hiddenDuringProductRun = useAuiState(state => state.message.status?.type === 'running')

  return toolViewMode === 'product' && hiddenDuringProductRun ? null : <MarkdownText />
}

const PresentedReasoningPart: ReasoningMessagePartComponent = props => {
  const toolViewMode = useStore($toolViewMode)

  return toolViewMode === 'technical' ? <ReasoningTextPart {...props} /> : null
}

const PresentedReasoningGroup: FC<PropsWithChildren<{ endIndex: number; startIndex: number }>> = props => {
  const toolViewMode = useStore($toolViewMode)

  return toolViewMode === 'technical' ? <ReasoningAccordionGroup {...props} /> : null
}

const PresentedToolPart: FC<ToolCallMessagePartProps> = props => {
  const toolViewMode = useStore($toolViewMode)
  ...
  const pendingApproval = Boolean(approval) && part.result === undefined && APPROVAL_TOOLS.has(part.toolName)

  return toolViewMode === 'technical' || pendingApproval || shouldPresentProductTool(part) ? (
    <ChainToolFallback {...props} />
  ) : null
}

const PresentedToolGroup: FC<PropsWithChildren<{ endIndex: number; startIndex: number }>> = ({
  children,
  endIndex,
  startIndex
}) => {
...
export const MESSAGE_PARTS_COMPONENTS = {
  Reasoning: PresentedReasoningPart,
  ReasoningGroup: PresentedReasoningGroup,
  Text: PresentedTextPart,
  ToolGroup: PresentedToolGroup,
  tools: { Fallback: PresentedToolPart }
} as const

...
describe('Hermes REST helpers', () => {
  ...
  it('targets a named profile for MCP list, replace, and delete', async () => {
    api.mockResolvedValue({ servers: [], ok: true })

    await listMcpServers('inbox')
    await saveMcpServers({ filesystem: { command: 'npx' } }, 'inbox')
    await removeMcpServer('composio', 'inbox')

    expect(api).toHaveBeenNthCalledWith(1, expect.objectContaining({ path: '/api/mcp/servers', profile: 'inbox' }))
    expect(api).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        ...
        method: 'PUT',
        profile: 'inbox',
        body: { servers: { filesystem: { command: 'npx' } } }
      })
    )
    expect(api).toHaveBeenNthCalledWith(
      3,
      expect.objectContaining({
        path: '/api/mcp/servers/composio',
        method: 'DELETE',
        profile: 'inbox'
      })
    )
